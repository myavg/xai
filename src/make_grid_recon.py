...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--recon_dir", type=str, default="outputs/baseline/recon")
    parser.add_argument("--output_path", type=str, default="outputs/baseline/grids/sanity_input_vs_recon.jpg")
...
def main() -> None:
    ...
    recon = collect_images(args.recon_dir, "recon")

    n = min(len(inputs), len(recon))
    if args.max_images is not None:
        n = min(n, args.max_images)
    if n == 0:
        raise RuntimeError("No paired input/reconstruction images found")
    ...
    for i in range(n):
        ...
        rec = Image.open(recon[i]).convert("RGB").resize((w, h))
